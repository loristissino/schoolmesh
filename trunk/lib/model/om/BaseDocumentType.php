...
abstract class BaseDocumentType extends BaseObject  implements Persistent {
	...
	public function setDescription($v)
	{
		...
		if ($this->description !== $v) {
			$this->description = $v;
			$this->modifiedColumns[] = DocumentTypePeer::DESCRIPTION;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = DocumentTypePeer::NUM_COLUMNS - DocumentTypePeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(DocumentTypePeer::DESCRIPTION)) $criteria->add(DocumentTypePeer::DESCRIPTION, $this->description);
	 ...
	 * Otherwise if this DocumentType has previously been saved, it will retrieve
	 * related Appointments from storage. If this DocumentType is new, it will return
		...
		if ($this->collAppointments === null) {
			if ($this->isNew()) {
			   $this->collAppointments = array();
				...
				$criteria->add(AppointmentPeer::DOCUMENT_TYPE_ID, $this->id);
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this DocumentType has previously been saved, it will retrieve
	 * related WpitemTypes from storage. If this DocumentType is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collWpitemTypes === null) {
			if ($this->isNew()) {
			   $this->collWpitemTypes = array();
				...
				$criteria->add(WpitemTypePeer::DOCUMENT_TYPE_ID, $this->id);
