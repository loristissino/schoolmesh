...
class Odf
{
    ...
    public function setVars($key, $value, $encode = true, $charset = 'ISO-8859')
    {
		// begin Loris' patch --
		$found=false;
        if (strpos($this->contentXml, $this->config['DELIMITER_LEFT'] . $key . $this->config['DELIMITER_RIGHT']) > 0) {
            $found=true;
        }
        if (strpos($this->stylesXml, $this->config['DELIMITER_LEFT'] . $key . $this->config['DELIMITER_RIGHT']) > 0) {
            $found=true;
        }
        if (!$found) {
