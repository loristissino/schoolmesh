...
class Segment implements IteratorAggregate, Countable
{
	...
	
    {
        if (strpos($this->xml, $this->odf->getConfig('DELIMITER_LEFT') . $key . $this->odf->getConfig('DELIMITER_RIGHT')) === false) {
			/* Loris' patch -- some field need to be optional
		...
		/*
		Loris' patch: I need to change also <br />, <em>, <sup> and <sub>
        $this->vars[$this->odf->getConfig('DELIMITER_LEFT') . $key . $this->odf->getConfig('DELIMITER_RIGHT')] = str_replace("\n", "<text:line-break/>", $value);

        */
		
		$text=str_replace(array('\n', '&lt;br /&gt;'), "<text:line-break/>", $value);
//		$text=str_replace('&lt;em&gt;', '**', $text);
//		$text=str_replace('&lt;/em&gt;','**', $text);

		$text=str_replace('&lt;em&gt;', '<text:span text:style-name="schoolmeshem">', $text);
		$text=str_replace('&lt;/em&gt;','</text:span>', $text);
		$text=str_replace('&lt;sup&gt;', '<text:span text:style-name="schoolmeshsup">', $text);
		$text=str_replace('&lt;/sup&gt;','</text:span>', $text);
		$text=str_replace('&lt;sub&gt;', '<text:span text:style-name="schoolmeshsub">', $text);
		$text=str_replace('&lt;/sub&gt;','</text:span>', $text);
		$text=str_replace('&lt;pagebreak&gt;', '</text:p><text:p text:style-name="schoolmeshpagebreak">', $text);

		$this->vars[$this->odf->getConfig('DELIMITER_LEFT') . $key . $this->odf->getConfig('DELIMITER_RIGHT')] = $text;
