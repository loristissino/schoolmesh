...
//from http://simplythebest.net/scripts/ajax/ajax_password_strength.html
  ...
  A.extend(
    A.fn, {
      pstrength:function(B){
        var B=A.extend({
          verdects:["Molto debole","Debole","Media","Forte","Molto forte"],
          colors:["#f00","#c06","#f60","#3c0","#3f0"],
          scores:[10,15,30,40],
          common:["password","sex","god","123456","123","liverpool","letmein","qwerty","monkey","prova","asdfghjkl"],
          minchar:8
          },B);
        return this.each(
          function(){
            var C=A(this).attr("id");
            A(this).after("<div class=\"pstrength-minchar\" id=\""+C+"_minchar\">Numero minimo di caratteri: "+B.minchar+"</div>");
            ...
            function(D,F,C){
              ...
              if(nPerc==-200){
                strColor="#f00";
                strText="Password non sicura!";
                A(B).css({width:"0%"})}
              else{if(nPerc<0&&nPerc>-199){
                strColor="#666";
                strText="Troppo corta";
                A(B).css({width:"5%"})}
