...
class SimpleSAML_Auth_Simple {
    ...
    SimpleSAML_Logger::debug('Called Simple: ' . var_export($authSource, TRUE));
    ...
    SimpleSAML_Logger::debug('Called requireAuth()');

		$session = SimpleSAML_Session::getInstance();
    SimpleSAML_Logger::debug('Got session');

		if ($session->isValid($this->authSource)) {
      SimpleSAML_Logger::debug('Already authenticated, returning');
			/* Already authenticated. */
			return;
		}

    SimpleSAML_Logger::debug('Going to login');
		$this->login($params);
	}
