...
*/
    public function executeIndex($request)
  {
    ...
    public function executePdf($request)
  {
  ...
  return sfView::NONE;
    ...
    public function executePdf2($request)
  {
	$config = sfTCPDFPluginConfigHandler::loadConfig();
  sfTCPDFPluginConfigHandler::includeLangFile($this->getUser()->getCulture());
 
  $doc_title    = "Titolo tutto mio";
  ...
  return sfView::NONE;
    ...
    public function executeRtf($request)
  {
/*
    $response = $this->getResponse();
    $response->clearHttpHeaders();
    $response->setContentType('text/rtf');
    $response->setHttpHeader('content-length','2057');
    $response->setHttpHeader('Content-Disposition','attachment; filename=data.rtf');
    $this->setLayout(false);

	readfile('/tmp/prova.rtf');
	return sfView::NONE;
*/
...
/** QUESTO FUNZIONA 
    ...
    header('Content-Disposition: attachment; filename=pianolavoro.rtf');
    header('Content-Length: 2057');
    readfile('/tmp/prova.rtf');
    // Make sure that no template is loaded. Instead, the file is served directly to the client.
    return sfView::HEADER_ONLY;
**/
/** QUESTO NON FUNZIONA 
    $this->getResponse()->clearHttpHeaders();
    $this->getResponse()->setContentType('text/rtf');
    $this->getResponse()->setHttpHeader('Content-Disposition', 'attachment; filename=pianolavoro.rtf');
    $this->getResponse()->setHttpHeader('Content-Length', '2057');
