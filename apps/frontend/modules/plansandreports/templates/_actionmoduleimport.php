			<ul class="sf_admin_td_actions">
				...
				<li class="sf_admin_action_import">
					<?php echo link_to(
				__('Import'),
				'plansandreports/importmodulefromdb?id=' . $workplan->getId() . '&module=' . $wpmodule->getId(), 
				array('method' => 'put', 'title'=>__('Import this module from the database'))
				)?>
				</li>
				<?php if(!$wpmodule->getAppointmentId()): ?>
				<li class="sf_admin_action_link">
					<?php echo link_to(
				__('Link'),
				'wpmodule/link?id=' . $wpmodule->getId() . '&workplan=' . $workplan->getId(), 
